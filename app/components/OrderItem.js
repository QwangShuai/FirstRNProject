            ...
            return (
                <View style={{backgroundColor: '#fff'}}>
                    ...
                    <View style={{height:UtilScreen.getHeight(44),marginBottom:UtilScreen.getHeight(20),flexDirection:'row',}}>
                        <Text style={[Styless.styles.travel_textView,{position:'absolute',right:UtilScreen.getWidth(340)}]}>邀请</Text>
                        <Text style={[Styless.styles.travel_textView,{position:'absolute',right:UtilScreen.getWidth(190)}]}>取消行程</Text>
                        <Text style={[Styless.styles.travel_textStyle,{position:'absolute',right:UtilScreen.getWidth(40)}]}>待支付</Text>
                    </View>
                </View>
            )
        } else if(this.props.itemInfo.state==='待行程'){
            if (this.props.itemInfo.payState) {
                return (
                    <View style={{backgroundColor: '#fff'}}>
                        <HeadView itemInfo={this.props.itemInfo} isShow={this.state.isShow} />
                        <View style={{height:UtilScreen.getHeight(44),marginBottom:UtilScreen.getHeight(20),flexDirection:'row',}}>
